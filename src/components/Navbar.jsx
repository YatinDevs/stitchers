        ...
        <div className="container mx-auto py-4 px-4">
          <div className="flex justify-between items-center">
                  ...
                  <button
                    ...
                    className={`px-4 py-2 rounded-lg flex items-center gap-1 transition-all ${
                      window.location.pathname === item.path
                        ? `text-[#436f6e] bg-[#e0dbdb]`
                        : "hover:text-[#436f6e] hover:bg-[#e0dbdb]/50"
                    }`}
                  >
                    {item.label}
                  </button>
            ...
            <div className="flex items-center gap-4">
              <button
                onClick={toggleCart}
                className="relative p-2 text-gray-700 hover:text-[#436f6e] transition-colors"
              >
                {itemCount > 0 && (
                  <span className="absolute -top-1 -right-1 bg-[#436f6e] text-white text-xs font-bold rounded-full w-5 h-5 flex items-center justify-center">
                    {itemCount}
                  </span>
                )}
              </button>
              ...
              <button
                onClick={toggleMenu}
                className="lg:hidden p-2 text-gray-700 hover:text-[#436f6e] transition-colors"
              >
      ...
      {isMenuOpen && (
        <div className="fixed top-0 right-0 w-full max-w-sm h-full bg-white z-[110] shadow-xl p-6 space-y-4 overflow-y-auto">
          <div className="flex justify-between items-center mb-8">
            ...
            <button
              onClick={toggleMenu}
              className="text-gray-800 hover:text-[#436f6e] p-2"
            >
              <X size={28} />
            </button>
          </div>
            ...
            {navItems.map((item, index) => (
              <div key={index} className="border-b border-gray-100 pb-2">
                <button
                  ...
                  className={`w-full text-left font-medium text-gray-800 py-3 flex justify-between items-center hover:text-[#436f6e]`}
                >
          ...
          {/* Mobile Contact Info */}
          <div className="mt-8 p-4 bg-[#e0dbdb] rounded-lg">
